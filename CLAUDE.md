...
**Recipe data** (`data/dish-<id>.json`): `dish.html` scales ingredient quantities live by `servings / dish.servings`. The `cookware` field is a **language-neutral code** at the top level (alongside `main`/`uses`), resolved to a name via `data/cookware.json` (`{ en/zh: { CODE: name } }`, same shape as `units.json`) — add new codes there. `cookware` is either a single `"CODE"` or, for a cookware chooser, an array `["CODE", …]`; in the array case `dish.html` renders a picker and each step phase may carry a `"cookware": "CODE"` so it shows only for that choice (phases with no `cookware` always show). `recipe.html` filters the dish grid by cookware (chips built from the codes dishes actually use).
